...
var spinX = 0;    // Rotation around the X-axis
var spinY = 0;    // Rotation around the Y-axis
var origX;        // Original mouse X position
var origY;        // Original mouse Y position
var movement = false;  // To track if the mouse is being moved while clicked
...
window.onload = function init()
{
    ...
    window.addEventListener("keydown", function(e){
        switch( e.keyCode ) {
            ...
            case 56:	// 8: from beside the car
                ...
                break;
            case 57:	// 9: from house(-40, 140)
                view = 9;
                document.getElementById("Viewpoint").innerHTML = "9: frá húsi í (-40,140)";
                break;
    ...
    canvas.addEventListener("mousedown", function(e) {
        ...
        origX = e.offsetX;
        origY = e.offsetY;
        e.preventDefault();  // Disable drag and drop behavior
    });
...
function render()
{
	...
	case 8:
	    ...
	    break;
    case 9:
        // View from a house at (-40, 140), always looking at the car
        mv = lookAt( vec3(-40.0, 140.0, 17.0 + height), // Camera position on top of the house
        vec3(carXPos, carYPos, 0.0), // Looking at the car's position
        vec3(0.0, 0.0, 1.0) ); // Up vector
        drawScenery(mv); // Draw the scenery from this view
        mv = mult(mv, translate(carXPos, carYPos, 0.0)); // Move the car
        mv = mult(mv, rotateZ(-carDirection)); // Rotate the car according to its direction
        drawCar(mv); // Draw the car with the new transformations
        break;
